import { DatabaseService, InputOutputService } from './service'


class Application {
    private menu: Array<string> = ["[0] Register", "[1] Make a purchase", "[2] My purchase history", "[3] End"];
    private selection = 0;
    ...
    private showPurchaseHistory() {
        ...
        history.forEach(data => this.inputOutputService.outputMessage("purchase history", data))
    }
...
const sysy = new Application(InputOutputService.getInstance(), DatabaseService.getInstance())
sysy.run()